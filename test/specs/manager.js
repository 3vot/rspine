describe("Manager", function(){
  ...
  beforeEach(function(){
    Users = RSpine.Controller.sub();
    Groups = RSpine.Controller.sub();

    users = new Users();
    groups = new Groups();
  });

  it("shouldn't overwrite existing methods in RSpine.Stack.constructor()", function(){
    var BadStack = RSpine.Stack.sub({
      controllers: {
        manager: Users
      }
    });
  ...
  it("should toggle active class", function(){
    new RSpine.Manager(users, groups);
  ...
  it("deactivate should work", function(){
    var manager = new RSpine.Manager(users, groups);
    users.active();
    manager.deactivate();
    expect(users.el.hasClass('active')).not.toBeTruthy();
  });
